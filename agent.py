...
async def _targeted_queries(question: str, answer: str, nb: Notebook, deadline: float) -> tuple[list[str], str]:
    """Ask for a compact audit plan: missing evidence queries plus rewrite notes."""
    if _left(deadline) < 56:
        return [], ""
    flags = _question_flags(question)
    prompt = (
        "Audit this draft against the question and evidence. Return JSON only: "
        "{\"queries\":[up to 3 web searches for missing/weak proof],"
        "\"notes\":\"short rewrite guidance\"}. "
        "Prefer authoritative roster/table/primary-source searches. If the draft is already fully supported, use an empty query list.\n\n"
        f"QUESTION:\n{question}\n\nFLAGS: {flags}\n\nDRAFT:\n{_clip(answer, 9000)}\n\nEVIDENCE:\n{nb.render(36000)}"
    )
    raw = await _chat([{"role": "user", "content": prompt}], deadline, 1200, 22.0, 0.0)
    obj = _json_from(raw)
    ...
    if isinstance(obj.get("queries"), list):
        for item in obj["queries"]:
            q = _clean(str(item))[:260]
            if q and q.lower() not in [x.lower() for x in queries]:
                queries.append(q)
    notes = str(obj.get("notes") or "")[:1200]
    return queries[:MAX_REPAIR_SEARCHES], notes


async def _targeted_gather(question: str, queries: list[str], nb: Notebook, deadline: float) -> None:
    """Second evidence wave used only after a draft reveals weak spots."""
    if not queries or _left(deadline) < 44:
        return
    search_tasks = [asyncio.create_task(_search_one(q)) for q in queries[:MAX_REPAIR_SEARCHES]]
    done, pending = await asyncio.wait(search_tasks, timeout=min(18.0, max(5.0, _left(deadline) - 35)))
    terms = _terms(question + " " + " ".join(queries))
    candidates: list[Row] = []
    for task in done:
        if task.exception():
            continue
    ...
    for task in pending:
        task.cancel()
    candidates.sort(key=lambda r: (_score_text(r.title + " " + r.url + " " + r.text[:1600], terms),
                                   any(x in _host(r.url) for x in ("gov", "edu", "int", "sec.gov", "official"))), reverse=True)
    fetch_tasks = []
    for row in candidates[:MAX_REPAIR_FETCHES]:
        if row.url.startswith(("http://", "https://")):
            fetch_tasks.append(asyncio.create_task(_fetch_one(row, question + " " + " ".join(queries))))
    if not fetch_tasks:
        return
    done, pending = await asyncio.wait(fetch_tasks, timeout=min(22.0, max(5.0, _left(deadline) - 18)))
    for task in done:
        if not task.exception():
            got = task.result()
            if got:
...
def _answer_prompt(question: str, nb: Notebook, repair: str = "") -> list[dict[str, Any]]:
    ...
    system = (
        ...
        "If a question has multiple conditions, test candidates against each condition rather than guessing from one clue. "
...
async def _solve(q: Query) -> Response:
    ...
    queries, audit_notes = await _targeted_queries(question, answer, nb, deadline)
    await _targeted_gather(question, queries, nb, deadline)
    if queries or audit_notes:
        refreshed = await _chat(_answer_prompt(question, nb, audit_notes or "Use the newly gathered evidence to strengthen weak citations."),
                                deadline, 4300, min(CHAT_S, 36.0), 0.02)
        if _usable(refreshed):
            answer = refreshed
